...
export const dbService = {
  // --- HERO ---
  async getHero() {
    return fetch("/api/data?key=hero", { cache: "no-store" }).then(handleResponse);
  },
  ...
  // --- THEMES ---
  async getThemes() {
    return fetch("/api/data?key=themes", { cache: "no-store" }).then(handleResponse);
  },
  ...
  // --- PRICING ---
  async getPricing() {
    return fetch("/api/data?key=pricing", { cache: "no-store" }).then(handleResponse);
  },
  ...
  // --- ADD-ONS ---
  async getAddons() {
    return fetch("/api/data?key=addons", { cache: "no-store" }).then(handleResponse);
  },
  ...
  // --- PORTFOLIO ---
  async getPortfolio() {
    return fetch("/api/data?key=portfolio", { cache: "no-store" }).then(handleResponse);
  },
  ...
  async getTestimonials() {
    return fetch("/api/data?key=testimonials", { cache: "no-store" }).then(handleResponse);
  },
  ...
  async getContact() {
    return fetch("/api/data?key=contact", { cache: "no-store" }).then(handleResponse);
  },
  ...
  // --- SEO ---
  async getSeo() {
    return fetch("/api/data?key=seo", { cache: "no-store" }).then(handleResponse);
  },
  ...
  // --- WEBSITE SETTINGS ---
  async getSettings() {
    return fetch("/api/data?key=settings", { cache: "no-store" }).then(handleResponse);
  },
  ...
  // --- MEDIA LIBRARY ---
  async getMedia() {
    return fetch("/api/media", { cache: "no-store" }).then(handleResponse);
  },
  ...
  // --- FOUNDER ---
  async getFounder() {
    return fetch("/api/data?key=founder", { cache: "no-store" }).then(handleResponse);
  },

  async updateFounder(data) {
    return fetch("/api/data?key=founder", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(data),
    }).then(handleResponse);
  ...
  // --- TEAM MEMBERS ---
  async getTeam() {
    return fetch("/api/data?key=team", { cache: "no-store" }).then(handleResponse);
  },

  async addTeamMember(member) {
    return fetch("/api/data?key=team", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  async updateTeamMember(id, member) {
    return fetch(`/api/data?key=team&id=${id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(member),
    }).then(handleResponse);
